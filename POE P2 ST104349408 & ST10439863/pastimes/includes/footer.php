<?php

?>
<footer class="site-footer">

  
  <div class="footer-brand">
    <a href="index.php" class="logo" style="text-decoration:none;">
      <span class="logo-mark">P</span>
      <span class="logo-text">Past<em>imes</em></span>
    </a>
    <p>Pre-loved branded fashion - condition-rated, seller-verified and affordably priced for South African shoppers.</p>
  </div>

  
  <div class="footer-col">
    <h4>Help</h4>
    <ul>
      <li><a href="contact.php">Help Centre</a></li>
      <li><a href="seller-guide.php">Selling &amp; Buying</a></li>
      <li><a href="about.php">Staying Safe</a></li>
    </ul>
  </div>

  
  <div class="footer-col">
    <h4>Legal Bits</h4>
    <ul>
      <li><a href="about.php#terms">Terms &amp; Conditions</a></li>
      <li><a href="about.php#privacy">Privacy Policy</a></li>
      <li><a href="shipping.php">Shipping Policy</a></li>
      <li><a href="about.php#disputes">Dispute Policy</a></li>
    </ul>
  </div>

  
  <div class="footer-col">
    <h4>Account</h4>
    <ul>
      <li><a href="login.php">Sign in</a></li>
      <li><a href="register.php">Register</a></li>
      <li><a href="orders.php">My Orders</a></li>
      <li><a href="admin_login.php">Admin portal</a></li>
    </ul>
  </div>

</footer>

<div class="footer-bottom">
  <span>&copy; <?= date('Y') ?> Pastimes South Africa (Pty) Ltd</span>

  <div class="footer-social">
    
    <a href="#" aria-label="Facebook" class="social-icon">
      <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
        <path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z"/>
      </svg>
    </a>
    
    <a href="#" aria-label="X" class="social-icon">
      <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
        <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
      </svg>
    </a>
    
    <a href="#" aria-label="YouTube" class="social-icon">
      <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
        <path d="M22.54 6.42a2.78 2.78 0 0 0-1.95-1.96C18.88 4 12 4 12 4s-6.88 0-8.59.46A2.78 2.78 0 0 0 1.46 6.42 29 29 0 0 0 1 12a29 29 0 0 0 .46 5.58 2.78 2.78 0 0 0 1.95 1.95C5.12 20 12 20 12 20s6.88 0 8.59-.47a2.78 2.78 0 0 0 1.95-1.95A29 29 0 0 0 23 12a29 29 0 0 0-.46-5.58z"/>
        <polygon points="9.75 15.02 15.5 12 9.75 8.98 9.75 15.02" fill="white"/>
      </svg>
    </a>
    
    <a href="#" aria-label="LinkedIn" class="social-icon">
      <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
        <path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z"/>
        <rect x="2" y="9" width="4" height="12"/><circle cx="4" cy="4" r="2"/>
      </svg>
    </a>
  </div>
</div>
</div>
<script>
  // Init Lucide icons
  document.addEventListener('DOMContentLoaded', function() {
    if (typeof lucide !== 'undefined') lucide.createIcons();
  });
</script>
<script src="js/main.js"></script>
</body>
</html>
